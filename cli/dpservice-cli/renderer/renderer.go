...
func (t defaultTableConverter) securityAssociationTable(sas []api.SecurityAssociation) (*TableData, error) {
	headers := []any{"SPI", "Direction", "Algorithm", "SrcUnderlay", "DstUnderlay", "ReplayWindow", "ESN"}
	if t.Wide {
	...
	for i, sa := range sas {
		columns[i] = []any{sa.Spi, sa.Spec.Direction, sa.Spec.Algorithm, sa.SrcUnderlay, sa.DstUnderlay, sa.Spec.ReplayWindow, sa.Spec.Esn}
		if t.Wide {
