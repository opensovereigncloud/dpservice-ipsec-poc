...
def testDpService(build_path, print_header):
	...
	if not args.hw:
		...
		# instance drops, so only the files named here can run in this mode
		suites.append(TestSuite("ipsec", "Security Association lifecycle and encrypted round-trip",
			test_args + ['--ipsec'], ['xtratest_ipsec_grpc.py', 'xtratest_ipsec_dataplane.py',
									  'test_vf_to_vf_encap.py', 'xtratest_ipsec_esn.py',
									  'xtratest_ipsec_rekey.py', 'xtratest_ipsec_xfrm.py',
									  'xtratest_ipsec_policy.py']))
