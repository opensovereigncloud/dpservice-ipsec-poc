...
{
	const char *append_array[] = { next_node_name };
	rte_edge_t count;

	if (rte_node_edge_update(node->id, RTE_EDGE_ID_INVALID, append_array, 1) != 1) {
		DPNODE_LOG_ERR(node, "Cannot add edge", _DP_LOG_STR("peer_node", next_node_name));
		return DP_ERROR;
	}

	count = rte_node_edge_count(node->id);
	if (count <= 0) {
		DPNODE_LOG_ERR(node, "No edge added", _DP_LOG_STR("peer_node", next_node_name));
		return DP_ERROR;
	}
					  ...
					  const char *tx_node_name)
{
	...
	return dp_node_append_edge(node, &next_tx_indices[port_id], tx_node_name);
}
